...
def book_page(room_type_id):
    ...
    if request.method == "POST":
        ...
        addon_text = ", ".join(addon["label"] for addon in selected_addons)
        user_special_requests = request.form.get("special_requests", "").strip()
        combined_special_requests_parts = []
        if addon_text:
            combined_special_requests_parts.append(f"Add-ons: {addon_text}")
        if user_special_requests:
            combined_special_requests_parts.append(user_special_requests)
        combined_special_requests = " | ".join(combined_special_requests_parts)
